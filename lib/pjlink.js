...
PJLink.prototype._resetConnection = function(){
	...
	if(this._connection){
		...
		if (this._connection.connecting) {	// If timeout occurs while connecting e.g. host not available then destroy
			this._connection.destroy();		// because end will not work here
		} else {

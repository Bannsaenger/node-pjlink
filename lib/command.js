...
Command.MuteCommand = function(){
	...
	if(args.length>0){
		...
		// handling of extended mute status
		if(typeof args[0]=='number') {
			var validStates = [10, 11, 20, 21, 30, 31];
			if (validStates.includes(args[0])) {
				data = String(args[0]);
			} else {
				data = '30';
			}
...
Command.MuteCommand.prototype.formatResult = function(args){
	// Added extended mute status
	var mute = {audio: false, video: false, status: 30}
	if(args.length>0){
		args = args[0];
		if(typeof args==='string' && args.length>1){
